...
def message(
    ...
    *,
    ...
    value: dict[str, Any] = {
        ...
        "body": body
        if body is not None
        else {"text": "Secret Body", "html": "<p>Secret Body</p>"},
        "snippet": snippet,
        "attachments": attachments
        if attachments is not None
        else [
            {"id": "a1", "name": "secret.pdf", "type": "application/pdf", "size": 42}
        ],
    }
    if is_draft:
        value["isDraft"] = True
...
def create_wrapped_db(path: Path, rows: list[tuple[str, Any, int]]) -> None:
    ...
    conn.execute(
        "CREATE TABLE threads (thread_id TEXT PRIMARY KEY, json TEXT, sort INTEGER, in_spam_trash INTEGER DEFAULT 0)"
    )
    for thread_id, payload, sort_value in rows:
        raw = payload if isinstance(payload, str) else json.dumps(payload)
        conn.execute(
            "INSERT INTO threads(thread_id, json, sort, in_spam_trash) VALUES (?, ?, ?, 1)",
            (thread_id, raw, sort_value),
        )
    conn.commit()
    conn.close()
...
class TestRequestValidation:
    @pytest.mark.parametrize(
        "value",
        ["2026-01-01", "2026-01-01T00:00:00+00:00", "2026-1-1T00:00:00Z", "not-a-date"],
    )
    def test_rejects_non_exact_utc_z(
        self, cache_config: tuple[Path, Path], value: str
    ) -> None:
        result = reader.scan(
            since=value,
            before="2026-01-02T00:00:00Z",
    ...
    def test_rejects_inverted_or_empty_window(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        result = reader.scan(
    ...
    def test_rejects_unknown_malformed_and_duplicate_selectors(
        self,
        cache_config: tuple[Path, Path],
        accounts: list[str],
        threads: list[str],
        people: list[str],
        code: str,
    ...
    def test_omitted_accounts_means_all_configured_accounts(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        ...
        assert result["data"]["selectors"]["accounts"] == [
            "a@example.com",
            "b@example.com",
        ]
...
class TestSnapshotSafety:
    def test_snapshot_is_unlinked_read_only_immutable_and_query_only(
        self,
        cache_config: tuple[Path, Path],
        tmp_path: Path,
        ...
        create_wrapped_db(
            source, [("t", {"messages": [message("m", 1767225600000)]}, 1767225600000)]
        )
    ...
    def test_temp_mode_is_0600_and_cleanup_happens_on_open_failure(
        ...
        def observe_connect(database: str, *, uri: bool) -> sqlite3.Connection:
            ...
            link_counts.append(snapshot.stat().st_nlink)
            assert database.startswith("file:/dev/fd/")
        ...
        with pytest.raises(reader.ReaderError), reader.transient_connection(source):
            pass
        assert modes == [0o600]
        assert link_counts == [0]
        assert list(tmp_path.glob(".shm-reader-*")) == []

    def test_source_change_fails_closed_and_deletes_snapshot(
        self,
        cache_config: tuple[Path, Path],
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
...
class TestScanSemantics:
    def test_exact_window_thread_and_person_semantics_include_spam_trash(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        source, _ = cache_config
        start = 1767225600000
        ...
        rows = [
            ...
            (
                ...
                {
                    "messages": [
                        message("other", start + 2, sender="target@example.com")
                    ]
                },
                start + 2,
            ),
        ...
        assert result["status"] == "succeeded"
        assert [item["message_id"] for item in result["data"]["records"]] == [
            "at-start"
        ]
    ...
    def test_deterministic_global_order(self, cache_config: tuple[Path, Path]) -> None:
        ...
        create_wrapped_db(
            ...
            [
                (
                    "z-thread",
                    {"messages": [message("b", when), message("a", when)]},
                    when,
                )
            ],
        )
        create_wrapped_db(
            second, [("a-thread", {"messages": [message("z", when + 1)]}, when + 1)]
        )
        result = run_scan(accounts=[])
        identities = [
            (
    ...
    def test_real_cache_date_attachment_and_draft_shapes(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        source, _ = cache_config
        raw = message(
        ...
        raw["date"] = "2026-01-01T00:00:00.000Z"
        raw["superhumanOwnDraftId"] = "provider-draft"
        create_wrapped_db(
            source,
            [("t", {"messages": [raw]}, 1767225600000)],
        )
        record = run_scan()["data"]["records"][0]
        assert record["date"] == "2026-01-01T00:00:00.000Z"
        assert record["date_epoch_ms"] == 1767225600000
        assert record["attachments"][0]["attachment_id"] == "provider-attachment"
        assert record["draft"] == {
            "is_draft": True,
            "evidence": ["message.superhumanOwnDraftId"],
        }

    def test_draft_read_label_attachment_and_bcc_facts(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        source, _ = cache_config
        create_wrapped_db(
            ...
            [
                (
                    ...
                    {
                        "messages": [
                            message("draft", 1767225600000, is_draft=True, bcc=None)
                        ]
                    },
                    1767225600000,
                )
        ...
        assert record["draft"] == {
            "is_draft": True,
            "evidence": ["message.isDraft", "message.draftId"],
        }
        assert record["read"] == {"is_unread": True, "evidence": "message.labelIds"}
        assert record["addresses"]["bcc"] == []
        assert record["attachments"] == [
            {"size_bytes": 42, "media_type": "application/pdf", "attachment_id": "a1"}
        ]


class TestProjectionAndCaps:
    def test_metadata_recursively_excludes_content_and_fts_fields(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        source, _ = cache_config
        create_wrapped_db(
    ...
    def test_full_content_has_provenance_and_complete_unavailable_truncated_states(
        ...
        payload = {
            "messages": [
                ...
                message(
                    ...
                    subject="",
                    body={},
                    snippet="must never become body",
                ),
            ]
        }
        create_wrapped_db(source, [("t", payload, 1767225600002)])
        result = run_scan(projection="full")
        records = result["data"]["records"]
        assert records[0]["content"]["subject"] == {
            "value": "abc",
            "coverage": "truncated",
            "provenance": "message.subject",
        }
        assert result["data"]["coverage"] == "truncated"
        assert result["data"]["accounts"][0]["coverage"] == "truncated"
        assert "FIELD_CONTENT_LIMIT" in result["data"]["truncation_reasons"]
        assert records[0]["content"]["body"]["text"] == {
            "value": "real",
            "coverage": "complete",
            "provenance": "message.body.text",
        }
        assert records[0]["content"]["body"]["html"] == {
            "value": None,
        ...
        assert (
            records[0]["addresses"]["from"][0]["display_name"]["coverage"] == "complete"
        )
        assert records[0]["attachments"][0]["filename"]["coverage"] == "complete"

    def test_full_fallback_provenance_names_actual_provider_fields(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        source, _ = cache_config
        fallback = message(
            ...
            1767225600000,
            body="plain body",
            attachments=[{"id": "a", "filename": "fallback.txt", "size": 1}],
        )
        fallback["from"] = {"email": "sender@example.com", "raw": "Raw Sender"}
        create_wrapped_db(
            source,
            [("t", {"messages": [fallback]}, 1767225600000)],
        )
        record = run_scan(projection="full")["data"]["records"][0]
        assert (
            record["addresses"]["from"][0]["display_name"]["provenance"]
            == "message.from.raw"
        )
        assert (
            record["attachments"][0]["filename"]["provenance"]
            == "message.attachments.filename"
        )
        assert record["content"]["body"]["text"]["provenance"] == "message.body"

    def test_global_content_and_output_caps_are_explicit(
        self, cache_config: tuple[Path, Path], monkeypatch: pytest.MonkeyPatch
    ) -> None:
        source, _ = cache_config
        ...
        payload = {
            "messages": [
                message(
                    f"m-{index}", 1767225600000 + index, body={"text": "abcdefghij"}
                )
        ...
        assert (
            content_result["data"]["records"][0]["content"]["subject"]["value"]
            == "Secre"
        )
        assert (
            content_result["data"]["records"][0]["content"]["subject"]["coverage"]
            == "truncated"
        )
        ...
        assert all(
            set(record)
            >= {"account_id", "thread_id", "message_id", "addresses", "attachments"}
            for record in output_result["data"]["records"]
        )

    def test_participant_attachment_record_and_record_caps_are_explicit(
        self, cache_config: tuple[Path, Path], monkeypatch: pytest.MonkeyPatch
    ) -> None:
        source, _ = cache_config
        ...
        assert result["data"]["coverage"] == "truncated"
        assert set(result["data"]["truncation_reasons"]) >= {
            "PARTICIPANT_LIMIT",
            "ATTACHMENT_LIMIT",
            "RECORD_LIMIT",
        }
...
class TestFailuresAndCLI:
    @pytest.mark.parametrize(
        "payload",
        ["{bad json", json.dumps({"messages": [42]}), json.dumps({"not_messages": []})],
    )
    def test_malformed_json_fails_whole_command_privately(
        self, cache_config: tuple[Path, Path], payload: str
    ) -> None:
        source, _ = cache_config
    ...
    def test_metadata_executes_one_threads_query_and_never_fts(
        self, cache_config: tuple[Path, Path], monkeypatch: pytest.MonkeyPatch
    ) -> None:
        source, _ = cache_config
        create_wrapped_db(
        ...
        assert all(
            "thread_search" not in sql.casefold() and "fts" not in sql.casefold()
            for sql in statements
        )

    def test_malformed_database_and_selected_account_failure_are_whole_command(
        self, cache_config: tuple[Path, Path]
    ) -> None:
        first, second = cache_config
        create_wrapped_db(
            first, [("t", {"messages": [message("m", 1767225600000)]}, 1767225600000)]
        )
    ...
    def test_schema_and_reader_dispatch(
        ...
        scan.assert_called_once_with(
            ...
            projection="metadata",
            threads=["t"],
            people=["p@example.com"],
        )
        assert json.loads(capsys.readouterr().out)["command"] == "reader.scan"

    def test_cli_missing_required_argument_is_reader_scan_envelope(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
